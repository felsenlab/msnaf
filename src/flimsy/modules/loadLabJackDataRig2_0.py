...
@param("channel_map", description="YAML config file containing column name: common name pairs") ## TODO -- maybe get rid of this cause we can just take fieldnames in the later modules (NOTE: this is made annoying by the O/0 confusion)
def run(data, params):
    ...
    for channel_name, column_name in channel_map.items():
        ...
        logger.warning(column_name)
        if channel_name in labjack_data:
