...
@requires("file_pattern", description="String that will be used to identify deeplabcut output files. The pipeline is designed to load the h5 output, rather than csvs. The wildcard operator * matches any number of characters (including none) in a file name, so a pattern like video_pattern='*DLC*.csv' will select all files that contain 'DLC' and end with '.csv', such as '20251217_unitME_session009_leftCam-0000DLC_resnet101_sacnetJan29shuffle1_1030000.csv' or '20251217_unitME_session009_rightCam-0000DLC_resnet101_sacnetJan29shuffle1_1030000.csv'; similarly, ? matches exactly one character.")
@requires("mapfile", description="Path to a configuration yaml file that defines the columns output by the deeplabcut model and the desired dataset fieldnames.") ## TODO -- may not need this if we can use the header rows to generate the pose fields
