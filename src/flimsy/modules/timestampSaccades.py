...
# @requires("frames/right/timestamps")
...
def run(data, params):
    ...
    timestamps = data["labjack/cameras/timestamps"]
    ...
    logger.debug("saccade timestamping debug")
    logger.debug(left_saccade_epochs)
    logger.debug(range(0, len(timestamps)))
    logger.debug(timestamps)

    left_saccade_timestamps = np.interp(left_saccade_epochs, range(0, len(timestamps), timestamps))
    right_saccade_timestamps = np.interp(right_saccade_epochs, range(0, len(timestamps), timestamps))
