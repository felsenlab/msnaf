...
@requires("video_pattern", description="String that will be used to identify video files. The wildcard operator * matches any number of characters (including none) in a file name, so a pattern like file_pattern='*Cam*.mp4' will select all files that contain 'Cam' and end with '.mp4', such as '20251112_unitME_session002_leftCam-0000.mp4' or '20251112_unitME_session002_rightCam-0000.mp4'; similarly, ? matches exactly one character.")
@requires("dlc_path", description="Path to the deeplabcut config file for the model to be used for analysis. This is typically a config.yaml in the deeplabcut project folder.")
